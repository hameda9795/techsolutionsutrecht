...
export const metadata: Metadata = {
  ...
  description: "Website laten maken in IJsselstein? Professionele WordPress websites vanaf €250. Snelle oplevering binnen 1 week. Ook laptop & telefoon reparatie. Gratis offerte!",
  keywords: ["website laten maken ijsselstein", "webdesign ijsselstein", "wordpress ijsselstein"],
...
export default function IJsselsteinPage() {
  const structuredData = {
    ...
    "@type": "LocalBusiness",
    "name": "TechSolutionsUtrecht - IJsselstein",
    "description": "Website laten maken en tech reparaties in IJsselstein",
    "url": "https://techsolutionsutrecht.nl/ijsselstein",
    "telephone": "+31625518708",
    "address": { "@type": "PostalAddress", "addressLocality": "IJsselstein", "addressRegion": "Utrecht", "addressCountry": "NL" },
    "geo": { "@type": "GeoCoordinates", "latitude": "52.0200", "longitude": "5.0431" },
    "serviceArea": { "@type": "City", "name": "IJsselstein" },
    "priceRange": "€€",
    "openingHours": "Mo-Fr 09:00-18:00"
  };

  const services = [
    { title: "WordPress Websites", price: "€250 - €800", description: "Professionele websites voor ZZP'ers en kleine bedrijven in IJsselstein.", features: ["Responsive design", "SEO basis inclusief", "Zelf te beheren"], link: "/diensten/wordpress" },
  ...
  const faqs = [
    { q: "Wat kost een website laten maken in IJsselstein?", a: "Onze WordPress websites beginnen vanaf €250. Dit is inclusief design, ontwikkeling, SEO-basis en oplevering." },
    { q: "Hoe snel kan mijn website online zijn?", a: "Een standaard website is binnen 3-10 dagen online." },
    { q: "Kom je ook langs in IJsselstein?", a: "Ja, we bedienen de hele regio Utrecht inclusief IJsselstein." },
        ...
        <nav className="py-4 bg-[var(--bg-secondary)] border-b border-[var(--border)]">
          <div className="container-custom">
            <ol className="flex items-center gap-2 text-sm flex-wrap">
              ...
              <li><span className="text-[var(--text-primary)] font-medium capitalize">ijsselstein</span></li>
            </ol>
          </div>
        </nav>
        <section className="pt-32 pb-20 bg-[var(--bg-primary)] relative overflow-hidden"><div className="absolute inset-0 hero-bg opacity-30" /><div className="container-custom relative z-10"><div className="max-w-4xl mx-auto text-center"><div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-4 py-2 rounded-full text-sm font-medium mb-6"><MapPin className="w-4 h-4" /><span>IJsselstein & Omgeving</span></div><h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-[var(--text-primary)] mb-6">Website Laten Maken <span className="gradient-text">IJsselstein</span></h1><p className="text-xl text-[var(--text-secondary)] mb-8 max-w-2xl mx-auto">Professionele WordPress websites voor ondernemers in IJsselstein. Vanaf <strong className="text-primary">€250</strong>, binnen <strong className="text-primary">1 week</strong> online.</p><div className="flex flex-wrap justify-center gap-4 mb-8"><div className="flex items-center gap-2 bg-[var(--bg-card)] px-4 py-2 rounded-full border border-[var(--border)]"><Star className="w-4 h-4 text-primary fill-primary" /><span className="text-sm">4.9/5 beoordeling</span></div><div className="flex items-center gap-2 bg-[var(--bg-card)] px-4 py-2 rounded-full border border-[var(--border)]"><CheckCircle className="w-4 h-4 text-green-500" /><span className="text-sm">20+ tevreden klanten</span></div><div className="flex items-center gap-2 bg-[var(--bg-card)] px-4 py-2 rounded-full border border-[var(--border)]"><Clock className="w-4 h-4 text-secondary" /><span className="text-sm">12+ jaar ervaring</span></div></div><div className="flex flex-col sm:flex-row gap-4 justify-center"><Link href="/contact" className="btn-primary group">Gratis offerte aanvragen<ArrowRight className="w-5 h-5 ml-2 transition-transform group-hover:translate-x-1" /></Link><a href="https://wa.me/31625518708" target="_blank" rel="noopener noreferrer" className="btn-ghost"><Phone className="w-5 h-5 mr-2" />WhatsApp direct</a></div></div></div></section>

        <section className="py-20 bg-[var(--bg-secondary)]"><div className="container-custom"><div className="text-center mb-12"><h2 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] mb-4">Onze Diensten in IJsselstein</h2><p className="text-[var(--text-secondary)] max-w-2xl mx-auto">Van eenvoudige websites tot complete webshops.</p></div><div className="grid md:grid-cols-3 gap-8">{services.map((service, index) => (<div key={index} className="card-hover bg-[var(--bg-card)] rounded-xl p-6 border border-[var(--border)]"><h3 className="text-xl font-semibold text-[var(--text-primary)] mb-2">{service.title}</h3><p className="text-primary font-bold text-lg mb-4">{service.price}</p><p className="text-[var(--text-secondary)] mb-4">{service.description}</p><ul className="space-y-2 mb-6">{service.features.map((feature, i) => (<li key={i} className="flex items-center gap-2 text-sm text-[var(--text-secondary)]"><CheckCircle className="w-4 h-4 text-green-500" />{feature}</li>))}</ul><Link href={service.link} className="btn-secondary w-full text-center">Meer info</Link></div>))}</div></div></section>

        <section className="py-20 bg-[var(--bg-primary)]"><div className="container-custom"><div className="grid md:grid-cols-2 gap-12 items-center"><div><h2 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] mb-6">Website Laten Maken in IJsselstein</h2><p className="text-[var(--text-secondary)] mb-4">IJsselstein is een historische stad met een rijk verleden. Met het karakteristieke centrum rond de Sint-Nicolaaskerk.</p><p className="text-[var(--text-secondary)] mb-6">Als tech-partner helpen wij ondernemers in IJsselstein met een professionele online presence.</p><div className="flex flex-wrap gap-2">{["Centrum", "Eiteren", "Zenderveen", "Arkelsteen", "Binnenstad"].map((area) => (<span key={area} className="bg-[var(--bg-tertiary)] px-3 py-1 rounded-full text-sm text-[var(--text-secondary)]">{area}</span>))}</div></div><div className="bg-[var(--bg-card)] rounded-2xl p-8 border border-[var(--border)]"><h3 className="text-xl font-semibold text-[var(--text-primary)] mb-4">Wij Bedienen Ook:</h3><ul className="space-y-3">{[{ name: "Utrecht", link: "/utrecht" }, { name: "Nieuwegein", link: "/nieuwegein" }, { name: "Houten", link: "/houten" }, { name: "De Meern", link: "/de-meern" }, { name: "Vleuten", link: "/vleuten" }].map((location) => (<li key={location.name}><Link href={location.link} className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-primary transition-colors"><MapPin className="w-4 h-4" />Website laten maken {location.name}</Link></li>))}</ul></div></div></div></section>
